...
<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std">
	<fieldset>
		<p class="text">
			...
			<input type="text" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email|escape:'htmlall'|stripslashes}{/if}" /><input type="submit" style="display:inline; margin-left:10px" class="button" value="{l s='Retrieve'}" />
		</p>
	</fieldset>
</form>
{/if}
<p class="clear">
	<a href="{$base_dir_ssl}authentication.php" title="{l s='Back to Login'}"><img src="{$img_dir}icon/my-account.png" alt="{l s='Back to Login'}" class="icon" /></a><a href="{$base_dir}authentication.php" title="{l s='Back to Login'}">{l s='Back to Login'}</a>
</p>
